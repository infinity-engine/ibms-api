...
cellInfoRoute.post("/", async (req, res) => {
  try {
    ...
    const assignedUsers = (users || [])
      ...
      .map(user => ({ _id: mongoose.Types.ObjectId(user._id), accessType: "write" }));
    ...
    const updateUserAccessOnCell = cellsInserted.map(cell => ({ _id: cell._id, accessType: "admin" }));
    const updateUserAccessOnCellOther = cellsInserted.map(cell => ({ _id: cell._id, accessType: "write" }));

    await Promise.all([
      USER.updateOne({ _id: req.user._id }, { $push: { configuredCells: updateUserAccessOnCell } }),
      ...(assignedUsers.map(user => USER.updateOne({ _id: user._id }, { $push: { configuredCells: updateUserAccessOnCellOther } }))),
    ]);
...
async function getCellsForUser(user) {
  ...
  const cells = await Cell.find({ _id: { $in: cellIds } })
